...
const BUSINESS_CATEGORIES: { value: BusinessCategoryType | ""; label: string }[] = [
  ...
  { value: "NIGHTLIFE_CLUB_EVENTS", label: "Nightlife, Club & Events" },
...
const TEAL_GRADIENT: readonly [string, string] = ["#14B8A6", "#2DD4BF"];
...
export default function BusinessSignupScreen() {
  ...
  const [businessCategory, setBusinessCategory] = useState<BusinessCategoryType | "">("");
  const [bio, setBio] = useState("");
  const [dateEstablished, setDateEstablished] = useState<Date | null>(null);
  ...
  const handleDateChange = (event: any, selectedDate?: Date) => {
    ...
    if (selectedDate) {
      setDateEstablished(selectedDate);
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (dateEstablished) {
        signupData.dateEstablished = dateEstablished.toISOString();
      }

      if (contactEmail.trim()) {
        signupData.contactEmail = contactEmail.trim();
      }

      if (contactPhone.trim()) {
        signupData.contactPhone = contactPhone.trim();
      }

      if (contactAddress.trim()) {
        signupData.contactAddress = contactAddress.trim();
      }
      ...
      if (whatsappNumber.trim()) {
        signupData.whatsappNumber = whatsappNumber.trim();
      }

      if (businessHours.trim()) {
        signupData.businessHours = businessHours.trim();
      }
  ...
  const getCategoryLabel = (value: string): string => {
    const option = BUSINESS_CATEGORIES.find((o) => o.value === value);
    return option?.label || "Select Business Category";
  };
            ...
            <View style={[styles.inputInner, multiline && styles.inputInnerMultiline]}>
              {icon ? (
                <Feather
                  name={icon as any}
                  size={20}
                  color={value ? theme.teal : theme.textTertiary}
    ...
    <GradientBackground variant="mesh" style={styles.container}>
      ...
      >
        ...
        >
          ...
          >
            Grow your brand and connect with clients
          </ThemedText>
        </Animated.View>
        ...
        >
              ...
              >
                <View style={styles.inputInner}>
                  <Feather
                    name="at-sign"
                    size={20}
                    color={username ? theme.teal : theme.textTertiary}
              ...
              <View style={styles.suggestionsContainer}>
                ...
                <View style={styles.suggestionsRow}>
                  {usernameSuggestions.slice(0, 3).map((suggestion) => (
                    ...
                    >
                      <ThemedText
                        style={[styles.suggestionText, { color: theme.teal }]}
                      >
                        {suggestion}
                      </ThemedText>
                    </Pressable>
            ...
            >
              <View
                style={[
                  ...
                  useEmail && { backgroundColor: theme.teal },
                ]}
              >
                <ThemedText
                  style={[
          ...
          <ThemedText
            style={[styles.sectionLabel, { color: theme.teal }]}
          >
              ...
              >
                <View style={styles.inputInner}>
                  <Feather
                    name="grid"
                    size={20}
                    color={businessCategory ? theme.teal : theme.textTertiary}
                    style={styles.inputIcon}
                  />
                  <ThemedText
                    style={[
                      styles.pickerText,
                      {
                        color: businessCategory ? theme.text : theme.textTertiary,
                        fontFamily: "Poppins_400Regular",
                      },
                    ]}
                  >
              ...
              >
                <View style={styles.inputInner}>
                  <Feather
                    name="calendar"
                    size={20}
                    color={dateEstablished ? theme.teal : theme.textTertiary}
                    style={styles.inputIcon}
                  />
                  <ThemedText
                    style={[
                      styles.pickerText,
                      {
                        color: dateEstablished ? theme.text : theme.textTertiary,
                        fontFamily: "Poppins_400Regular",
                      },
                    ]}
                  >
                    {dateEstablished
                      ? formatDate(dateEstablished)
          ...
          {Platform.OS === "ios" ? (
            ...
            >
              ...
              <Picker
                ...
                onValueChange={(value) => {
                  setBusinessCategory(value as BusinessCategoryType | "");
              ...
              <Pressable
                ...
                style={[styles.modalDoneButton, { backgroundColor: theme.teal }]}
                onPress={() => setShowCategoryPicker(false)}
              >
          ...
          >
            <View style={styles.modalOverlay}>
              ...
              >
                ...
                <DateTimePicker
                  testID="date-picker"
                  value={dateEstablished || new Date()}
                  mode="date"
                  display="spinner"
