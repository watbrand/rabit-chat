...
async function downloadFileWithRetry(url, outputPath, maxRetries = 3) {
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      await downloadFileSingle(url, outputPath);
      return; // Success
    } catch (error) {
      lastError = error;
...
async function downloadFileSingle(url, outputPath) {
  const controller = new AbortController();
  const timeoutId = setTimeout(() => controller.abort(), 180_000); // Increased to 3 minutes
  ...
  try {
    ...
    if (error.name === "AbortError") {
      throw new Error(`Download timeout after 3m: ${url}`);
    }
    throw error;
  } finally {
