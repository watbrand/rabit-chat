...
export const chatStorage: IChatStorage = {
  ...
  async createConversation(title: string) {
    const [conversation] = await db.insert(conversations).values({ title } as any).returning();
    return conversation;
  },
  ...
  async createMessage(conversationId: string, role: string, content: string) {
    const [message] = await db.insert(messages).values({ conversationId, role, content } as any).returning();
    return message;
  },
};
